function Annotation_Matrix = MEP_Visualizer(data, config)
    ...
    % - Use 'r' to modify a Peak Window for an individual trial. 
    ...
    %   config.TriggerTime       = 0; % Occurence of Pulse/Trigger; Time in [mS] relative to the first sample; If provided, this is used as reference point for the PeakWindow
    %   config.PeakWindow        = [20 40] % Time Window for calculation of peak to peak amplitude; >> Time in [mS] relative to Trigger Time<<
    ...
    %   Annotation Type (=1, 2, 3), Onset of PeakWindow, Offset of PeakWindow, Peak-to-peak Amplitude
        ...
        viz.Data = data;
        viz.NumChannels = 0;
        viz.Xlimit = nan; % Time in MilliSeconds
        ...
        % Set X-Axis Values
        viz.Current.Time = (linspace(0,size(viz.Data,1)/viz.SamplingFrequency.*1000,size(viz.Data,1)) - viz.TriggerTime);% in ms
    ...
    buttons = {'w', 's', 'a','d', 'q', 'e', 'r', 'p', 't', 'm', 'n', 'h', '1', '2', '3', '0', '9', 'escape'};
    ...
    fprintf('r: select a window to compute peak-to-peak amplitude (drag over area) \n')
