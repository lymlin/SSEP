function newsig = initFFTfilter(wave,range)
% Input: wave for filtering
%        bandpass range
%        save freq
% Output: filtered data
    BandpassRange = [range(1),range(2)];%0.25-4000
    wave  = reshape(wave,[1],[]);
    N = length(wave);
    ...
    FreqSpecAnalysis(newsig);
    print('Frequency spectrum - notch filtered and artifacts removed.tif','-dtiffn')
end